package com.nanodegree.abhinav.titanfilms.mostpopular.adapter;
...
/**
 * This class is MostPopularGridView Adapter.
 *
 * Created by Abhinav Puri
 */
public class MostPopularMoviesGridAdapter extends BaseAdapter {

    /**
     * Context for MostPopularMovies Activity.
     */
    private Context context;

    /**
     * Layout Inflater from MostPopularMovies Activity.
     */
     ...
     */
    private MostPopularMoviesDataFetchHandler mostPopularMoviesDataFetchHandler;
     ...
     */
    public MostPopularMoviesGridAdapter(Context context, LayoutInflater layoutInflater, ArrayList<TheMovieDBObject> theMovieDBObjectArrayList){
     ...
     *
     * @param mostPopularMoviesDataFetchHandler
     */
    public void setHandler(MostPopularMoviesDataFetchHandler mostPopularMoviesDataFetchHandler){
        this.mostPopularMoviesDataFetchHandler = mostPopularMoviesDataFetchHandler;
    }

    /**
     * Getter for list of TheMovieDBObject.
     * @return list of most popular movies data.
     */
    public ArrayList<TheMovieDBObject> getTheMostPopularMoviesData(){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(!isDummyData) {
            ...
            if(isFirstTime && applicationSavedData.getCurrentSelectedForMostPopular()==null){
                applicationSavedData.setCurrentSelectedForMostPopular(theMovieDBObjectArrayList.get(position));
            } else if(isFirstTime && applicationSavedData.getCurrentSelectedForMostPopular()!=null) {
                if (applicationSavedData.getCurrentSelectedForMostPopular().getTitle() == null) {
                    applicationSavedData.setCurrentSelectedForMostPopular(theMovieDBObjectArrayList.get(position));
                }
            }
            ...
            if(isFirstTime) {
                if (context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE &&
                        ((Activity) context).getFragmentManager().findFragmentById(R.id.most_popular_fragment_2) != null) {
                    TheMostPopularDetailsFragment fragment = (TheMostPopularDetailsFragment) ((Activity) context).getFragmentManager().findFragmentById(R.id.most_popular_fragment_2);
                    fragment.refresh(true);
                }
                isFirstTime = false;
            }

            if(applicationSavedData.getCurrentSelectedForMostPopular().equals(theMovieDBObjectArrayList.get(position))){
            ...
            convertView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if(context.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE &&
                        ...
                        Intent intent = new Intent(context, MostPopularMoviesDetailActivity.class);
            ...
            if(position == theMovieDBObjectArrayList.size()-10) {
                // time to load more ;
                mostPopularMoviesDataFetchHandler.loadMore();
            }
